...
export default function EditResourcesArticleDialog({ resourcesArticleId, initialTitle, initialContent, initialLink }: EditResourcesArticleDialogProps) {
    ...
    useEffect(() => {
        ...
        setLink(initialLink);
    }, [initialTitle, initialContent, initialLink]);
    ...
    const handleSubmit = async (e?: React.FormEvent) => {
        ...
        if (initialTitle === title && initialContent === content && initialLink === link) {
            return;
        }
        ...
        try { 
            const result = await updateResourcesArticle(resourcesArticleId, title, content, link); 
            ...
            <form onSubmit={handleSubmit}> 
                ...
                <DialogContent className="sm:max-w-[425px]"> 
                    ...
                    <DialogFooter> 
                        ...
                        <LoadingButton type="submit" onClick={handleSubmit} disabled={(!title.trim()) || (!content.trim()) || (!link.trim()) || (initialTitle === title && initialContent === content && initialLink === link) || isLoading} loading={isLoading}>Save changes</LoadingButton>
                    </DialogFooter> 
                </DialogContent> 
            </form> 
        </Dialog> 
